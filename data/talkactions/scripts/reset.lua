...
function onSay(cid, words, param)
    ...
    if config.redskull and getCreatureSkullType(cid) == 4 then
        return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ORANGE, "You need to be without red skull to reset.")
    elseif config.pz and not getTilePzInfo(getCreaturePosition(cid)) then
        return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ORANGE, "You need to be in protection zone to reset.")
    elseif config.battle and getCreatureCondition(cid, CONDITION_INFIGHT) then
        return
            doPlayerSendTextMessage(cid, "You need to be without battle to reset.")
    end
    ...
    if getPlayerLevel(cid) < resetLevel then
        return doPlayerSendTextMessage(cid, MESSAGE_EVENT_ORANGE, "You need level " .. resetLevel .. " or more to reset.")
    end
    ...
    doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Now you have " .. getPlayerResets(cid) .. " " .. (getPlayerResets(cid) == 1 and "reset" or "resets") .. ".")
    return true
end
